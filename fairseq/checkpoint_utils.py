...
def _upgrade_state_dict(state):
    ...
    for key in state.keys():
        if "debert" in key:
            debert_flag = True
            break
        if "bert" in key or "electra" in key:
            bert_flag = True
            if "generator_predictions" in key:
                electra_gen = True

    if bert_flag:
        new_state = {}
        new_state["model"] = state
        new_state["optimizer_history"] = [{"criterion_name": "CrossEntropyCriterion", "best_loss": 0.0}]
        new_state["extra_state"] = {"epoch": 0, "batch_offset": 0, "val_loss": 0.0, "train_iterator": {"epoch": 0,"iterations_in_epoch": 0,}}
        new_state["args"] = Namespace()
        new_state["args"].task = "bert_gen_translation"
        if electra_gen:
            new_state["args"].arch = "bert_genator_transformer"
        else:
            new_state["args"].arch = "bert_gen_transformer"
        new_state["args"].dataset_impl = "lazy"
        new_state["extra_state"]["train_iterator"]["epoch"] = 1
        new_state["cfg"] = convert_namespace_to_omegaconf(new_state["args"])
        return new_state
    elif debert_flag:
        new_state = {}
        new_state["model"] = state
        ...
        new_state["extra_state"] = {"epoch": 0, "batch_offset": 0, "val_loss": 0.0, "train_iterator": {"epoch": 0,"iterations_in_epoch": 0,}}
        new_state["args"] = Namespace()
        new_state["args"].task = "deberta_gen_translation"
        new_state["args"].arch = "deberta_dynamic_transformer"
        new_state["args"].dataset_impl = "lazy"
        new_state["extra_state"]["train_iterator"]["epoch"] = 1
        new_state["cfg"] = convert_namespace_to_omegaconf(new_state["args"])
        return new_state
